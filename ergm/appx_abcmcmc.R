...
path = paste0('appx/abcmcmc/E', eps_threshold, 'm', cycle)
ifelse(!dir.exists(path), dir.create(path), FALSE)
filename = paste0(path, '/appx', repID, '.RData')
...
#==============================================================================-
niter = nrow(postSamp$theta)
nn = 200000
postSamp = postSamp$theta[(burn+1+1):(burn+nn+1),]
