...
// [[Rcpp::export]]
List ergmABCMCMC(mat X, mat COV, mat theta, int outer, int cycle, double epsilon, double eps_threshold){
  
  double logprob, u, distl;                        
  int nCOVcols = COV.n_cols, percentile, nAdap = 200;                
  vec thetaprev(nCOVcols);                  
  vec stat = Summary(X), statprop(nCOVcols); 
  vec dummyvec;
  ...
  double rhat, gamma1, gamma2, c0 = 1, c1 = 0.8, ropt = 0.23, sigma2 = 1;
  int adapIter = 1;
  mat cholCOV = chol(COV) + 0.00000000001 * diagmat(ones(nCOVcols)); 
  
  for(int l = 0; l< outer; l++){

    // if( (l > 1000) && (l <= 10000) ){
    //   COV = cov(theta) + 0.00000000001 * diagmat(ones(nCOVcols));
    // }
    
    if( (l >= nAdap) && (l % nAdap == 0) ){
      rhat = sum( accprob.rows(l-nAdap, l-1) ) / nAdap;
      gamma1 = 1 / pow(adapIter, c1);
      gamma2 = c0 * gamma1;
      sigma2 = exp( log(sigma2) + gamma2 * (rhat - ropt) );
      
      COV = COV + gamma1 * ( cov( theta.rows(l+1-nAdap, l) ) - COV );
      cholCOV = chol( sigma2 * ( COV + 0.0000000001 * diagmat(ones(nCOVcols)) ) );
      adapIter = adapIter + 1;
    }
    ...
    if( distl < epsilon ){
      ...
      logprob = ( -0.05*trans(thetaprop)*thetaprop + 0.05*trans(thetaprev)*thetaprev )[0];
  ...
  return List::create(Named("theta") = theta,
                      Named("COV") = COV,
                      Named("sigma2") = sigma2,
                      Named("adapIter") = adapIter,
                      Named("accprob") = accprob,
                      Named("count") = count,
                      Named("dist") = dist,
                      Named("distl") = distl,
